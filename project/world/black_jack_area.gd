...
func _ready() -> void:
	...
	_black_jack_requested.connect(get_tree().root.get_node('World')._on_black_jack_requested)


func _process(_delta: float) -> void:
	if Input.is_action_just_pressed("interact") and _in_area:
		_black_jack_requested.emit()


func _on_body_entered(_body: Node2D) -> void:
	_gamble_area_entered.emit("BLACKJACK!")
	_in_area = true
